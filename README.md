# Personal Event Pipeline — n8n + ZeroClaw on Sakura VPS

スマホ（Android / Tasker）から送信した個人イベントデータ（位置情報など）を収集・集約し、同一 VPS 上の ZeroClaw に圧縮して渡すことで、タイミングに応じたパーソナルなメッセージを受け取るためのシステム。
...
2. **Phase 2（ZeroClaw 連携の運用強化）**: 未実装
   - n8n から ZeroClaw 送信、再送ポリシーの明文化
...
```bash
docker compose up zeroclaw -d --build
```
...
※ localhost からアクセスするため、事前に docker-compose.yml の zeroclaw サービスに一時的に `ports: ["127.0.0.1:42617:42617"]` を追加するか、`docker compose exec` 経由で実行する。

4. 返されたトークンを `.env` の `ZEROCLAW_GATEWAY_TOKEN` に設定

5. チャネル設定（Telegram/Discord に配信する場合）

```bash
docker compose exec zeroclaw zeroclaw onboard --channels-only
```
...
3. ZeroClaw にテスト digest を直接送信

```bash
docker compose exec n8n \
  curl -X POST http://zeroclaw:42617/webhook \
    -H "Authorization: Bearer <ZEROCLAW_GATEWAY_TOKEN>" \
    -H "Content-Type: application/json" \
    -d '{"message": "[LocationDigest] 09:00 自宅出発 → 09:30 オフィス到着"}'
```

4. パイプライン全体のテスト（テストイベント → digest 生成 → ZeroClaw 送信）
...
curl -X POST http://localhost:3000/events \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "event_id": "test-zeroclaw-integration-001",
    "type": "location",
...
5. ZeroClaw のログで受信を確認

```bash
docker compose logs zeroclaw --follow
```
...
- ZeroClaw への送信は n8n から HTTP Request ノードで行う（Docker 内部ネットワーク経由）
...
- ZeroClaw 送信成功時に `digests.sent_at` を更新
...
- **圧縮してから ZeroClaw へ**: 生データは溜め、状態変化と短い要約だけを送る
- **5分以内の応答**: イベント発生から ZeroClaw のメッセージ受信まで最大5分（cron 間隔）
...
③ digest 生成 : ZeroClaw 向けの短いテキスト or JSON に変換 → digests テーブルへ INSERT
④ 状態更新    : 対象 events の processed_at を現在時刻で UPDATE
⑤ 送信        : digests WHERE sent_at IS NULL → ZeroClaw gateway へ POST
...
- Workflow B: `digest-send`
  - Trigger: Cron（1〜5分ごと）
  - 処理: 未送信 digest 取得 → ZeroClaw 送信 → 成功時のみ `digests.sent_at` 更新

> 集計と送信を分離し、ZeroClaw 停止時でも digest 生成処理を継続できる構成にする。

#### 2) 再送ポリシー（ZeroClaw 送信失敗時）
...
- [ ] ZeroClaw 停止中でも digest が生成され続け、復旧後に自動送信される
...
Authorization: Bearer <ZEROCLAW_GATEWAY_TOKEN>
...
- Docker 内部ネットワーク経由で送信（外部通信なし）
- ペアリングトークンで認証
- 失敗時は `digests.sent_at` が NULL のまま残り、次回バッチで再送
- `digest_id` を message に含めることで ZeroClaw 側で重複判定が可能
...
services:
  ...
  zeroclaw:
    ...
    volumes:
      ...
      - ./zeroclaw/identity.md:/etc/zeroclaw/IDENTITY.md:ro
...
```
n8n (内部) → HTTP Request → ZeroClaw gateway (Docker 内部ネットワーク)
```

- n8n が cron で DB をポーリングし digest を生成
- digest を ZeroClaw のゲートウェイに POST（Docker 内部通信、外部通信なし）
- ZeroClaw がペアリングトークンで認証
...
- 送信先が Docker 内部ネットワークになるため、ネットワーク起因の失敗は大幅に減る
- n8n の HTTP Request ノードで retry（指数バックオフ）を設定
- digest_id を message に含め、ZeroClaw 側で重複を認識可能にする
...
### Phase 2: ZeroClaw 統合の最適化

- [ ] ZeroClaw の identity.md を調整（未実装）
- [ ] タイミング制御（通勤中・帰宅後などに応じたメッセージング）（未実装）
- [ ] digest フォーマット最適化（未実装）
- [ ] pg_dump 日次バックアップの自動化（未実装）
- 次アクション: ZeroClaw 送信のリトライ方針（上限・間隔）を仕様化する。
...
| ZeroClaw の identity / メモリ設定の詳細設計 | digest の解釈ルール、応答テンプレート |
...
- **本番**: さくら VPS（Ubuntu）
- **ZeroClaw**: さくら VPS（Docker Compose 内）
